...
let
  # Переменные Шлюза 1
  localIp = "10.0.2.1";          # Внешний IP этого шлюза (Сеть 2)
  remoteIp = "10.0.2.2";         # Внешний IP второго шлюза
  remoteSubnet = "10.0.3.0/24";  # Внутренняя сеть за вторым шлюзом (Сеть 3)
  tunnelIpLocal = "172.16.0.1/23"; # IP туннеля для Шлюза 1
in
{
  networking.hostName = "gateway1";
  networking.useDHCP = false;

  # Форвардинг пакетов (обязательно для шлюза)
  boot.kernel.sysctl = { "net.ipv4.ip_forward" = 1; };

  # Физические интерфейсы
  networking.interfaces.enp0s3.ipv4.addresses = [{
    address = "192.168.10.1"; # Сеть 1 (Внутренняя)
    prefixLength = 24;
  }];
  networking.interfaces.enp0s8.ipv4.addresses = [{
    address = "10.0.2.1";     # Сеть 2 (Транзитная)
    prefixLength = 24;
  }];
  ...
  systemd.services = {
    ...
    tunnel-vti = {
      ...
      script = ''
        ...
        ip link set vti1 up
        # SPI 1 для исходящего, SPI 2 для входящего
